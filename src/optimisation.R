...
quantmod::getSymbols("EDV")
quantmod::getSymbols("SPY")

EDV.month<-to.monthly(EDV);SPY.month<-to.monthly(SPY)#;tail(SPY.month$SPY.Adjusted);tail(EDV.month$EDV.Adjusted)

rEDV<-returns(EDV.month$EDV.Adjusted)#; tail(rEDV);head(rEDV)
